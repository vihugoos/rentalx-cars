...
describe("List Car Images Controller", () => {
    beforeAll(async () => {
    ...
    it("Should be able to list car images", async () => {
        ...
        // List Car Images Controller (test ListCarImagesController)
        const response = await request(app).get(`/cars/images/${car.id}`);

        expect(response.status).toBe(200);
        expect(response.body.length).toBe(2);
        expect(response.body[0].image_name).toContain("ferrari-488-spider-1");
        expect(response.body[0].image_url).toContain("http://localhost:3333");
        expect(response.body[0].image_url).toContain("ferrari-488-spider-1");
